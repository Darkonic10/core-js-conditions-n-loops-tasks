...
function canQueenCaptureKing(queen, king) {
  ...
  for (let i = Math.min(minX, minY); i < Math.max(maxX, maxY); i += 1) {
    x += 1;
    y += 1;
    if ((x === maxX || minX === maxX) && (y === maxY || minY === maxY)) {
      return true;
    }
  }
  return false;
...
function convertToRomanNumerals(num) {
  ...
  let changedNum = num;
  let result = '';
  for (let i = 0; i < romanObj.length; i += 1) {
    const [roman, number] = romanObj[i];
    while (changedNum >= number) {
      result += roman;
      changedNum -= number;
    }
  }
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i <= size; i += 1) {
    initialMatrix[i] = [...matrix[i]];
  }

  for (let row = 0; row <= size; row += 1) {
    for (let col = 0; col <= size; col += 1) {
      rotatedMatrix[col][size - row] = initialMatrix[row][col];
    }
  }
...
function getNearestBigger(number) {
  ...
  const sortedTail = digits.splice(i).sort((a, b) => a - b);
  return +[...digits.splice(0, i), ...sortedTail].join('');
}
